...
DEFAULT_OUTPUT_PATH = ROOT / "docs" / "structured_signal_validation_provisional_report_001.md"
...
def build_report(payload: dict) -> str:
    ...
    lines = [
        "# Structured Signal 가정 반영 리포트 001",
        "",
        "- 이 문서는 현재 `structured_signal_validation_wave_001.csv`의 결정값을 그대로 적용했을 때의 미리보기입니다.",
        f"- 전체 항목: `{len(items)}`",
...
def main() -> None:
    ...
    print(content)
    print(f"Wrote structured signal provisional report to {output_path}")
